...
def load_conllu(file, skip_checks=False):
    ...
    while True:
        ...
        if not line:
            ...
            def process_word(word):
                if not skip_checks and word.parent == "remapping":
            ...
            if not skip_checks and len([word for word in ud.words[sentence_start:] if word.parent is None]) != 1:
...
def evaluate(gold_ud, system_ud):
    ...
    def alignment_score(alignment, key_fn=None, filter_fn=None,
        ...
        for words in alignment.matched_words:
            if filter_fn is None or filter_fn(words.gold_word):
                if (bypass_fn is not None and bypass_fn(words.gold_word)) or key_fn(words.gold_word, gold_aligned_gold) == key_fn(words.system_word, gold_aligned_system):
                    correct += 1
    ...
    return {
        ...
        "BLEX": alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL],
                                ...
                                filter_fn=lambda w: w.is_content_deprel),
        "UNK_LAS": alignment_score(alignment, lambda w, ga: (ga(w.parent),w.columns[DEPREL]),
            filter_fn=lambda w: w.columns[FORM] not in VOCAB),
        "KNOWN_LAS": alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL]),
            filter_fn=lambda w: w.columns[FORM] in VOCAB),
        "C_UNK_LAS": alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL]),
            bypass_fn=lambda w: w.columns[FORM] not in VOCAB),
        "C_KNOWN_LAS": alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL]),
            bypass_fn=lambda w: w.columns[FORM] in VOCAB),
    }


def load_conllu_file(path, skip_checks=False):
    _file = open(path, mode="r", **({"encoding": "utf-8"} if sys.version_info >= (3, 0) else {}))
    return load_conllu(_file, skip_checks)

def evaluate_wrapper(args):
    # Load CoNLL-U files
    gold_ud = load_conllu_file(args.gold_file, args.skip_checks)
    system_ud = load_conllu_file(args.system_file, args.skip_checks)
...
def main():
    ...
    if not args.verbose and not args.counts:
        ...
        print(f"Raw LAS: {evaluation['LAS'].f1}")
    else:
        if args.counts:
        ...
        for metric in["Tokens", "Sentences", "Words", "UPOS", "XPOS", "UFeats",
                ...
                "KNOWN_LAS", "UNK_LAS", "C_KNOWN_LAS", "C_UNK_LAS"]:
            if args.counts:
