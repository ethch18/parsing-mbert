...
class VariableSizeCharsVocabulary(UnicodeCharsVocabulary):
    """
    Flexible-length version of `UnicodeCharsVocabulary`.
    Adapted from the Rosita `PolyglotCharsVocabulary`.
    """
    ...
    def __init__(self, filename, max_word_length, validate_file=False):
        ...
        with open(filename) as f:
            for line in f:
                ...
                if word_name not in self._word_to_id:
                    self._id_to_word.append(word_name)
                    self._word_to_id[word_name] = idx
                    idx += 1
        ...
        for i, word in enumerate(self._id_to_word):
            for ch in word:
                char_counts[ch] += 1
            self.char_ids = {ch:i for i, ch in \
                    enumerate(sorted(char_counts.keys()))}
    ...
    def save_vocab(self, savedir):
        ...
        with open(os.path.join(savedir,"word_vocab.txt"),'w') as f:
            for word in self._word_to_id:
                f.write("{}\t{}\n".format(word,self._word_to_id[word]))
