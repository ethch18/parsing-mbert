...
from .scalar_mix_with_dropout import ScalarMixWithDropout
...
@TokenEmbedder.register("bert-pretrained-with-dropout")
class PretrainedBertEmbedder(BertEmbedder):
    ...
    def __init__(self, pretrained_model: str, requires_grad: bool = False, top_layer_only: bool = False,
                 layer_dropout: float = 0.0, scalar_mix_parameters: List[float] = None) -> None:
        model = PretrainedBertModel.load(pretrained_model)

        for param in model.parameters():
            param.requires_grad = requires_grad

        super().__init__(bert_model=model, top_layer_only=top_layer_only, scalar_mix_parameters=scalar_mix_parameters, layer_dropout=layer_dropout)