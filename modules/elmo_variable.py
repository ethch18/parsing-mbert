...
from .elmo_lstm_variable import ElmoLstm
from allennlp.modules.highway import Highway
from allennlp.modules.scalar_mix import ScalarMix
from allennlp.nn.util import remove_sentence_boundaries, add_sentence_boundary_token_ids, get_device_of
from .elmo_indexer_variable import ELMoCharacterMapper,\
        ELMoTokenCharactersIndexer, VariableLengthELMoCharacterMapper
...
class Elmo(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self._char_map_file = char_map_file
...
def batch_to_ids(batch: List[List[str]],
        char_map_file: str = None) -> torch.Tensor:
    """
    ...
    """
    instances = []
    indexer = ELMoTokenCharactersIndexer(char_map_file=char_map_file)
...
class _ElmoCharacterEncoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if char_map_file is not None:
            self._mapper_class = VariableLengthELMoCharacterMapper(char_map_file)
        else:
            self._mapper_class = ELMoCharacterMapper
        ...
        self._beginning_of_sentence_characters = torch.from_numpy(
                numpy.array(self._mapper_class.beginning_of_sentence_characters) + 1
        )
        self._end_of_sentence_characters = torch.from_numpy(
                numpy.array(self._mapper_class.end_of_sentence_characters) + 1
        )
...
class _ElmoBiLm(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self._token_embedder = _ElmoCharacterEncoder(options_file, weight_file,
                char_map_file=char_map_file, requires_grad=requires_grad)
        ...
        self._elmo_lstm = ElmoLstm(input_size=options['lstm']['projection_dim'],
                                   ...
                                   requires_grad=requires_grad,
                                   use_skip_connections=options['lstm']['use_skip_connections'])
    ...
    def create_cached_cnn_embeddings(self, tokens: List[str]) -> None:
        ...
        """
        tokens = [self._mapper_class.bos_token, self._mapper_class.eos_token] + tokens
        timesteps = 32
        batch_size = 32
